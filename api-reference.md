# OneKeyMiner 1.21.1 API Reference
...
- `onekeyminer-fabric-1.6.7-1.21.1.jar`
- `onekeyminer-forge-1.6.7-1.21.1.jar`
- `onekeyminer-neoforge-1.6.7-1.21.1.jar`
...
```groovy
dependencies {
    modCompileOnly files("libs/onekeyminer-fabric-1.6.7-1.21.1.jar")
}
```
...
```groovy
dependencies {
    compileOnly fg.deobf(files("libs/onekeyminer-forge-1.6.7-1.21.1.jar"))
}
```
...
```groovy
dependencies {
    compileOnly files("libs/onekeyminer-neoforge-1.6.7-1.21.1.jar")
}
```
...
ticks. Protocol v3 uses versioned payload identifiers on Fabric and an exact
version negotiation on Forge/NeoForge. The channels are optional for peers
without the mod, while a present but incompatible channel is never decoded.
Duplicate retries return the original acknowledgement; stale sequences and
sequence reuse with different contents are rejected. Add-ons should update
configuration through the API rather than sending additional protocol packets.
...
- API examples in this document target only Minecraft 1.21.1 / OneKeyMiner
