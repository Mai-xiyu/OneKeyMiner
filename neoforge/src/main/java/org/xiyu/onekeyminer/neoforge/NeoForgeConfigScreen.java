...
public class NeoForgeConfigScreen {
    ...
    private static class SimpleConfigScreen extends Screen {
        ...
        protected void init() {
            ...
            int discordButtonWidth = 90;
            int discordButtonHeight = 20;
            ...
            if (!serverSettingsEditable) {
                initClientPreferencesPage(centerX, startY + spacing, buttonWidth, buttonHeight, spacing);
            } else {
                switch (currentPage) {
        ...
        private void initPageGeneral(int x, int y, int w, int h, int s) {
            ...
            addCycleButton(x, y + s * i++, w, h, "config.onekeyminer.option.max_blocks", () -> configCopy.maxBlocks, v -> configCopy.maxBlocks = v, new int[]{16, 32, 64, 128, 256, 512, 1000});
            addCycleButton(x, y + s * i++, w, h, "config.onekeyminer.option.max_distance", () -> configCopy.maxDistance, v -> configCopy.maxDistance = v, new int[]{8, 16, 32, 64});
            addBoolButton(x, y + s * i++, w, h, "config.onekeyminer.option.allow_diagonal", () -> configCopy.allowDiagonal, v -> configCopy.allowDiagonal = v);
        }

        private void initClientPreferencesPage(int x, int y, int w, int h, int s) {
            int i = 0;
            this.addRenderableWidget(Button.builder(
                    getShapeMessage(configCopy.selectedShape),
                    b -> {
        ...
        private void initPageConsumption(int x, int y, int w, int h, int s) {
            int i = 0;
            addBoolButton(x, y + s * i++, w, h, "config.onekeyminer.option.consume_durability", () -> configCopy.consumeDurability, v -> configCopy.consumeDurability = v);
            addBoolButton(x, y + s * i++, w, h, "config.onekeyminer.option.stop_low_durability", () -> configCopy.stopOnLowDurability, v -> configCopy.stopOnLowDurability = v);
            addCycleButton(x, y + s * i++, w, h, "config.onekeyminer.option.preserve_durability", () -> configCopy.preserveDurability, v -> configCopy.preserveDurability = v, new int[]{1, 5, 10, 20, 50});
            addBoolButton(x, y + s * i++, w, h, "config.onekeyminer.option.consume_hunger", () -> configCopy.consumeHunger, v -> configCopy.consumeHunger = v);
            addCycleButton(x, y + s * i++, w, h, "config.onekeyminer.option.min_hunger", () -> configCopy.minHungerLevel, v -> configCopy.minHungerLevel = v, new int[]{0, 2, 6, 10, 14});
            addBoolButton(x, y + s * i++, w, h, "config.onekeyminer.option.mine_all", () -> configCopy.mineAllBlocks, v -> configCopy.mineAllBlocks = v);
            addBoolButton(x, y + s * i++, w, h, "config.onekeyminer.option.allow_bare_hand", () -> configCopy.allowBareHand, v -> configCopy.allowBareHand = v);
        }
        ...
        private Component getBoolMessage(String key, boolean value) {
            return Component.translatable(key).append(": ").append(Component.translatable(value ? "gui.yes" : "gui.no").withStyle(value ? ChatFormatting.GREEN : ChatFormatting.RED));
        }
        ...
        public void extractRenderState(net.minecraft.client.gui.GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
            ...
            if (!serverSettingsEditable) {
                ...
                var acknowledgement = ClientPreferenceSession.lastAck();
                Component applied = acknowledgement.<Component>map(ack -> Component.translatable(
                ...
                guiGraphics.centeredText(this.font, applied, this.width / 2, 40, 0xAAAAAA);
            }
            guiGraphics.centeredText(this.font, Component.literal((currentPage + 1) + " / " + totalPages()), this.width / 2, this.height - 45, 0xAAAAAA);
        }
    }
}
