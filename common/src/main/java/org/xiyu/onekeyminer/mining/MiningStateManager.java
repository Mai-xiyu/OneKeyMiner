...
public final class MiningStateManager {
    ...
    public static void setTeleportDrops(UUID uuid, boolean enabled) {
        update(uuid, old -> new PlayerState(
                old.holding(), old.activated(), old.shape(), enabled, old.teleportExp()
        ));
    }
    ...
    public static void setTeleportExp(UUID uuid, boolean enabled) {
        update(uuid, old -> new PlayerState(
                old.holding(), old.activated(), old.shape(), old.teleportDrops(), enabled
        ));
    }

    /** Applies one validated preference snapshot without exposing partial state. */
