...
/** Shared validation and policy application for every loader adapter. */
public final class ClientPreferenceServer {
    private static final PreferencePacketGuard PACKET_GUARD =
            new PreferencePacketGuard(600);

    private ClientPreferenceServer() {
    }

    /** Returns {@code null} when a packet must be rejected without an ACK. */
    public static ClientPreferenceAck apply(
        ...
        if (wireVersion != ClientPreferenceProtocol.WIRE_VERSION || sequence <= 0) {
            if (PACKET_GUARD.shouldLogInvalid(player.getUUID(), tick)) {
                OneKeyMiner.LOGGER.warn(
                        "Ignoring invalid client preference packet from {} "
                                + "(wire={}, sequence={})",
        ...
        if (shapeId == null || !ShapeRegistry.isRegistered(shapeId)) {
            if (PACKET_GUARD.shouldLogInvalid(player.getUUID(), tick)) {
                OneKeyMiner.LOGGER.warn(
                        "Replacing invalid shape preference '{}' from {} "
