...
/** Per-player admission and invalid-packet log sampling. */
public final class PreferencePacketGuard {
    ...
    PreferencePacketGuard(long invalidLogIntervalTicks, int maxPacketsPerTick) {
        if (invalidLogIntervalTicks < 1 || maxPacketsPerTick < 1) {
            throw new IllegalArgumentException("packet guard limits must be positive");
        }
    ...
    public boolean shouldLogInvalid(UUID playerId, long tick) {
        ...
        lastInvalidLogTicks.compute(playerId, (ignored, previous) -> {
            if (previous == null || tick < previous || tick - previous >= invalidLogIntervalTicks) {
                shouldLog[0] = true;
                return tick;
            }
            return previous;
    ...
    public void clear(UUID playerId) {
        acceptedWindows.remove(Objects.requireNonNull(playerId, "playerId"));
