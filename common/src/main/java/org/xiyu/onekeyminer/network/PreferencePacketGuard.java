...
/** Per-player packet admission and invalid-packet log sampling. */
public final class PreferencePacketGuard {
    private final long invalidLogIntervalTicks;
    private final int maxPacketsPerTick;
    private final ConcurrentHashMap<UUID, TickWindow> acceptedWindows =
            new ConcurrentHashMap<>();
    private final ConcurrentHashMap<UUID, Long> lastInvalidLogTicks =
            new ConcurrentHashMap<>();
    ...
    PreferencePacketGuard(long invalidLogIntervalTicks, int maxPacketsPerTick) {
        if (invalidLogIntervalTicks < 1) {
            throw new IllegalArgumentException(
                    "invalidLogIntervalTicks must be positive"
            );
        }
