...
- `onekeyminer-fabric-1.6.8-1.21.9.jar` for Fabric
- `onekeyminer-neoforge-1.6.8-1.21.9.jar` for NeoForge
- `onekeyminer-forge-1.6.8-1.21.9.jar` for Forge
...
The two `allowClient*` options are authoritative server policies. Setting either
to `false` prevents clients from enabling that teleport behavior, even when the
corresponding client preference is `true`.
...
- **Tag format**: `<branch>-<mod_version>` (example: `1.21.9-1.6.8`).
